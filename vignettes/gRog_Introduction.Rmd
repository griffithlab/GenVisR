...
By default genCov will perform a log compression of genomic space for each feature type,'Intron','CDS','UTR'. The degree of compression can be set via the parameter `base` which will perform the appropriate log compression for the geatures specified in `transform`. The user can turn off this behavior by setting transform to NULL.
