...
#' @param domain_data object of class data frame specifying protien domain information, obtained from fetchDomain
...
construct_gene <- function(gene, domain_data, length)
{
  ##################################################################################
  ############### function to construct gene/domain data frame #####################
  ##################################################################################
  ...
  colnames(gene) <- c("Domain", "pos_from", "pos_to", "height_min", "height_max")
