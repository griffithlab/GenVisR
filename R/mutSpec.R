...
{ 
  ...
  # add in a count of mutations at the sample level before anything is stripped
  # out and save for mutation recurrence plot
  data_frame2 <- add_mutation_counts(data_frame[,c('sample',
                                                   'gene',
                                                   'trv_type')])
  ...
  if(!is.null(mutBurden))
  {
    ...
    {
      stop("levels in the sample column of mutBurden does not match either the
            the samples given in x or main.samples")
    }
  ...
  if(is.null(clinDat))
  {
    p1 <- build.main.mutSpec(data_frame, grid=main.grid, label_x = main.label_x,
                             gene_label_size=main.gene_label_size,
                             file_type = file_type, drop_mutation=drop_mutation,
                             plot_x_title=TRUE, plot_label=main.plot_label_flag,
