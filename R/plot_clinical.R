...
{
  # Define the theme
  theme <- theme(panel.grid.major = element_blank(), panel.grid.minor=element_blank(), panel.background=element_rect(fill='white', colour='white'), axis.ticks.x=element_blank(), axis.ticks.y=element_blank(), axis.text.x=element_blank(), axis.title.x=element_text(size=18), legend.title=element_blank(), axis.title.y=element_blank(), axis.text.y=element_text(size=16, colour='black'), legend.position='right', legend.text=element_text(size=16))
  ...
  p1 <- ggplot(x, aes(x=sample, y=variable, fill=value)) + geom_tile() + theme + leg_guide + fill + xlab('Sample n=625')
