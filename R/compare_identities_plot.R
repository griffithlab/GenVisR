...
#' @import reshape2
#' @import scales
compare_identities_plot <- function(count_tables, sample_names)
{
  ...
  ## Add a column of the variant allele after the column of the reference allele
  var <- c('A','A','C','T','G','C','C','A','C','C','G','A','G','T','G','A','A','A','A','C','C','G','G','C')
  ...
  vaf.long <- melt(vaf, id.vars='id')
  
  ## Plot VAF
  # The point represents the VAF (y-axis) for each dbSNP (x-axis). The colour of the 
  # points represent samples. Title "Variant Base" refers to the x-axis labels when 
  ...
  # Remove x-axis ticks.
  plotVAF <- plotVAF + theme_bw() + theme(legend.position="none", panel.grid.minor.y=element_blank(), panel.grid.minor.x=element_line(colour="grey"), plot.margin=unit(c(1,1,0,1), "cm"), axis.title.x=element_blank(), axis.ticks=element_blank())
  ...
  plotVAF <- gtable_add_rows(plotVAF, plotVAF$heights[plotVAF$layout[rn, ]$l], panel$t-1)
  plotVAF <- gtable_add_grob(plotVAF, axisb, l=panel$l, t=panel$t, r=panel$r)
  ...
  plot.df <- ggplot(.df, aes(x=id,y=a, label=ref)) + geom_blank() + xlab("Reference Base") + scale_x_continuous(limits=c(0.5,24.5), breaks=1:24) + scale_y_continuous(limits=c(0.99,1.01)) + theme_bw() + theme(plot.margin=unit(c(0,1,0,1), "cm"), axis.title.x=element_text(size=14, vjust=1), axis.title.y=element_blank(), axis.text.y = element_blank(), axis.text.x = element_blank(), axis.ticks=element_blank(), panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.border=element_blank())
  ...
  readcounts.long <- melt(readcounts.df, id.vars='id', measure.vars=sample_names)
  
  # Plot read counts
  dbSNP_rsID <- c('rs2229546','rs1410592','rs497692','rs10203363','rs2819561','rs4688963','rs309557','rs2942','rs17548783','rs4735258','rs1381532','rs10883099','rs4617548','rs7300444','rs9532292','rs2297995','rs4577050','rs2070203','rs1037256','rs9962023','rs2228611','rs10373','rs4148973','rs4675')
  plotRC <- ggplot(readcounts.long, aes(x=id, y=as.numeric(value), fill=variable)) + geom_bar(stat="identity",position="dodge")
  # x-axis: Label as "dbSNP rsID". Label ticks with rsIDs.
  plotRC <- plotRC + xlab("dbSNP rsID") + scale_x_continuous(limits=c(0.5,24.5), breaks=1:24, labels=dbSNP_rsID)
  # y-axis: Label as "Total Readcount". Make a logarithmic scale. Set breaks.
  plotRC <- plotRC + ylab("Total Readcount") + scale_y_continuous(trans=log10_trans(), breaks=c(10,50,100,500,1000))
  # theme: Make panel white. Move legend underneath graph. Rotate x-axis labels 90deg.
  plotRC <- plotRC + theme_bw() + theme(legend.position="bottom", axis.text.y=element_text(hjust=0.5), axis.text.x=element_text(angle=90, vjust=0.5, hjust=0.5), panel.grid.major.x = element_blank(), panel.grid.minor.x=element_line(colour="grey"), plot.margin=unit(c(0,1,1,1), "cm"))
  
  ## Graph plotVAF over plot.df over plotRC
  # Change widths of plot.df and plotRC to width of plotVAF
  plot.df <- ggplot_gtable(ggplot_build(plot.df))
  plot.df$widths <- plotVAF$widths
  plotRC <- ggplot_gtable(ggplot_build(plotRC))
  plotRC$widths <- plotVAF$widths
  
  # Plot all
  plotVAF_refbases <- arrangeGrob(plotVAF, plot.df, heights=c(88,12))
  plotVAFoverRC <- arrangeGrob(plotVAF_refbases, plotRC, heights=c(60,40))

  return(grid.draw(plotVAFoverRC))
}
