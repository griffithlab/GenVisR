...
build.cnFreq <- function(data_frame, plotType, plot_title=NULL, CN_low_colour='#002EB8', CN_high_colour='#A30000', x_lab_size=12, y_lab_size=12, facet_lab_size=10, layers=NULL)
{
  #x <- na.omit(data_frame) this causes problems converting factors to numbers
    x <- data_frame

  # Define Theme of plot
    theme <- theme(strip.text.x=element_text(size=facet_lab_size), axis.text.x=element_blank(), axis.ticks.x=element_blank(), legend.position='right', axis.title.x=element_text(size=x_lab_size, face='bold'), axis.title.y=element_text(size=y_lab_size, face='bold'))
  ...
  # Define main plot using boundaries in dummy data and then plot actual data
    ymax <- ifelse(plotType=="prop", 1, max(as.numeric(as.character(x$obs)),na.rm=TRUE))
