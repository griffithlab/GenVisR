...
build_track_name <- function(name, bg_fill="black", text_fill="white", border="black", size=10)
{
  ...
  label <- geom_text(aes(x=.5, y=.5), label=name, colour=text_fill, angle=90, size=size)
  ...
  theme <- theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank(), plot.margin=unit(c(0, 0, 0, 0), "null"), axis.ticks.length=unit(0,"null"), axis.ticks.margin=unit(0, "null"), panel.margin=unit(0,"null"))
  
  # Define the main plot
  label <- ggplot(mapping=aes(xmin=0, xmax=1, ymin=0, ymax=1)) + background + label + theme + scale_x + scale_y + labels
