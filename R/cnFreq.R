...
cnFreq <- function(x, CN_low_cutoff=1.5, CN_high_cutoff=2.5, plot_title=NULL, background='grey90', CN_low_colour='#002EB8', CN_high_colour='#A30000', x_lab_size=12, y_lab_size=12, facet_lab_size=10, layers=NULL)
{
  ...
  if(plotType=="freq")
  {
    ...
    for(i in 1:nrow(xuniq)) {
      ...
      gain[i] = sum(x[tmpind,"segmean"] >= CN_high_cutoff, na.rm=T)
      loss[i] = sum(x[tmpind,"segmean"] <= CN_low_cutoff, na.rm=T)
      obs[i] = length(tmpind)
    }
  ...
  # Construct the plot
  p1 <- build.cnFreq(x, plotType, plot_title=plot_title, background=background, CN_low_colour=CN_low_colour, CN_high_colour=CN_high_colour, x_lab_size=x_lab_size, y_lab_size=y_lab_size, facet_lab_size=facet_lab_size, layers=layers)
