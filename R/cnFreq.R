...
#'
#' given a matrix construct a plot to display copy number changes across the
#' genome for a group of samples
#' @name cnFreq
#' @param x object of class data frame with rows representing the proportion of
#' CN losses/gains across the genome (default), or actual CN values. The formor
#' option contains columns chromosome, start, end, gain, and loss, and the
#' latter option contains columns chromosome, start, end, segmean, and sample
#' @param CN_low_cutoff numeric value representing the cutoff for copy number
#' losses. Only used if x represents CN values.
#' @param CN_high_cutoff numeric value representing the cutoff for copy number
#' gains. Only used if x represents CN values.
...
#' @param CN_high_colour character string specifying high
#' value of colour gradient
                   ...
                   x_lab_size=12, y_lab_size=12, facet_lab_size=10, layers=NULL)
{
  ...
  # If x contains actual CN values, transform into frequencies
    ...
    p1 <- cnFreq_buildMain(x, plotType, plot_title=plot_title,
                           ...
                           CN_high_colour=CN_high_colour, x_lab_size=x_lab_size,
                           y_lab_size=y_lab_size, facet_lab_size=facet_lab_size,
                           layers=layers)
