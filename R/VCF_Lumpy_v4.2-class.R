...
#' Class VCF_Lumpy_v4.2
#' 
#' An S4 class to represent data in vcf version 4.2 format, inherits from the 
#' VCF_Virtual class
#' @name VCF_Lumpy_v4.2
#' @rdname VCF_Lumpy_v4.2-class
...
VCF_Lumpy_v4.2 <- function(vcfData, vcfHeader, paired, tumorColumn) {
    ...
    temp <- suppressWarnings(data.table::rbindlist(apply(vcfData, 1, function(x, paired) {
        ...
        a <- x$tumorSample
        x$total_read_support <- as.numeric(strsplit(a, ":")[[1]][2])
        x <- as.data.table(t(cbind(x)))
        
        ## Get the ID
        id <- strsplit(as.character(x$ID), split="_")[[1]][1]
        x$ID <- id
        return(x)
    }, paired=paired)))
    ...
    ## Remove duplicate IDs (i.e. reciprocal translocation events)
    vcfData <- vcfData[!duplicated(vcfData$ID),]
    ...
    new("VCF_Lumpy_v4.2", description=finalDescription, sample=sample, 
