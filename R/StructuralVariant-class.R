...
#' coordinates
#' @param filterSvCalls Boolean specifying if SV calls that did not pass should be removed
...
#' @sectionHeights Integer vector of length 3 specifying the relative heights for each of 
#' the plots (TRA, chr, non-TRA). The sum of the 3 integers must be 1. 
#' Defaults to 0.4, 0.1, and 0.5
...
StructuralVariant <- function(input, BSgenome=NULL, filterSvCalls=TRUE, svType=NULL,
                              ...
                              sampleColor=NULL, plotALayers=NULL, 
    ...
    structuralVariantPlots <- svPlots(object=svDataset, 
                                      plotSV=inputParameters@plotSV, 
                                      ...
                                      sample=inputParameters@sample, 
                                      sampleColor=inputParameters@sampleColor, 
          ...
          definition=function(object, plotSV, plotSpecificGene, plotTraGenes, plotOtherGenes, 
              ...
              if (plotSV == TRUE) {
                  ...
                  svPlots <- suppressWarnings(lapply(window, function(dataset, plotSpecificGene, 
                      ...
                      chr1End <- chr1Length
                      if (length(chrOrder) == 1) {
                          boundaries <- data.table(start=c(0), end=c(chr1End))
                      }
                      if (length(chrOrder) > 1) {
                          chr2End <- chr2Length
